...
import {
    ...
    updateProperty,
    jsonPath,
    ocapiGetCodeVersions,
    ocapiActiveCodeVersion,
    ocapiCreateDeleteCodeVersion,
    getProphetInfo,
...
export class CommandHandler {
    ...
    private async createCodeVersion() {
        ...
        if (!result.error) {
            ...
            vscode.window.showInformationMessage(
                `${Constants.CODEVERSION_SUCCESS_FIRST}${cvName}${Constants.INPUTBOX_SUCCESS_SECOND}${json.hostname}`,
            );
            this.refreshAllTreeViews();
        } else {
    ...
    private async deleteCodeVersion() {
        ...
        if (!result.error) {
            const json = defaultJson();
            vscode.window.showInformationMessage(
                `${Constants.CODEVERSION_SUCCESS_FIRST}${cvName}${Constants.DELETE_ITEM_SUCCESS_SECOND}${json.hostname}`,
            );
            this.refreshAllTreeViews();
        } else {
